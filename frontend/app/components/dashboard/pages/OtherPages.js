...
export function RiskMapPage() {
  ...
  const zones = [
    { id: "Z1", x: 80, y: 60, w: 160, h: 100, label: "Utilidades", risk: "low", equips: ["Bomba A-1 (IoT)", "Bomba A-2"], alerts: 0 },
    { id: "Z2", x: 280, y: 60, w: 140, h: 100, label: "Elétrica", risk: "medium", equips: ["Motor C-5", "Painel Elétrico"], alerts: 1 },
    { id: "Z3", x: 460, y: 60, w: 150, h: 100, label: "Geração", risk: "high", equips: ["Turbina T-01 (IoT)"], alerts: 2 },
    { id: "Z4", x: 80, y: 200, w: 200, h: 120, label: "Processo", risk: "medium", equips: ["Reator RX-2", "Válvula D-7"], alerts: 1 },
    { id: "Z5", x: 320, y: 200, w: 160, h: 120, label: "Compressores", risk: "high", equips: ["Compressor B-3 (IoT)"], alerts: 3 },
    { id: "Z6", x: 80, y: 360, w: 400, h: 80, label: "Instrumentação", risk: "low", equips: ["Sensor T-12 (IoT)", "Sensor P-08"], alerts: 0 },
  ];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {[["low", "Baixo"], ["medium", "Médio"], ["high", "Alto"]].map(([r, label]) => (
            <div key={r} style={{ display: "flex", alignItems: "center", gap: 4 }}>
              ...
              <span style={{ fontSize: "0.72rem", color: C.gray500 }}>{label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1, background: C.white, border: `1px solid ${C.gray200}`, borderRadius: 8, padding: "1rem", minWidth: 400 }}>
          <svg viewBox="0 0 680 480" style={{ width: "100%", cursor: "default" }}>
            ...
            {zones.map((zone) => {
              ...
              return (
                <g key={zone.id} onClick={() => setSelectedZone(isSelected ? null : zone)} style={{ cursor: "pointer" }}>
                  ...
                  <text x={zone.x + zone.w / 2} y={zone.y + 22} textAnchor="middle" fontSize="10" fontWeight="700" fill={r.label}>{zone.label}</text>
                  {zone.alerts > 0 && (
                    <g>
            ...
            <text x="65" y="478" fontSize="9" fill={C.gray400}>● Pulsação = IoT ativo | Número vermelho = alertas</text>
          </svg>
        </div>
        ...
        <div style={{ width: 240, display: "flex", flexDirection: "column", gap: 8 }}>
          {selectedZone ? (
            <div style={{ background: C.white, border: `1px solid ${C.gray200}`, borderRadius: 8, padding: "1rem" }}>
              ...
              {selectedZone.equips.map((e) => (
                <div key={e} style={{ display: "flex", alignItems: "center", gap: 6, padding: "0.4rem 0", borderBottom: `1px solid ${C.gray100}` }}>
                  <Ico name="box" size={12} color={C.gray400} />
                  <span style={{ fontSize: "0.78rem", color: C.gray600 }}>{e.replace(" (IoT)", "")}</span>
            ...
            <div style={{ background: C.gray50, border: `1px dashed ${C.gray200}`, borderRadius: 8, padding: "1.5rem", textAlign: "center" }}>
              ...
              <p style={{ margin: "0.5rem 0 0", fontSize: "0.78rem", color: C.gray400 }}>Clique em uma zona para ver detalhes</p>
            </div>
          )}
          ...
          <div style={{ background: C.white, border: `1px solid ${C.gray200}`, borderRadius: 8, padding: "0.85rem" }}>
            ...
            {zones.filter((z) => z.alerts > 0).map((z) => (
